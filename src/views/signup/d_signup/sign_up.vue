...
export default {
	...
	computed: {
		progress() {
			return this.user.name && this.user.email && this.authCode && this.user.password && this.user.password === this.password_Check ? 25 : 0;
		},
		isFormValid() {
			return this.user.name && this.user.email && this.authCode && this.user.password && this.user.password === this.password_Check;
		},
	},
	...
	methods: {
		...
		async IdCheck() {
			try {
				const regId = this.checkId(this.user.id)
				const response = await this.$axiosWithoutValidation.post(`/signUp/id/${regId}`)
		...
		async EmailCheck() {
			try {
				...
				const response = await this.$axiosWithoutValidation.post(`/signUp/email/${clearemail}`);
		...
		sendMail() {
			this.$axiosWithoutValidation.post(`/service/authemail/${this.user.email}`)
				.then((response) => {
					this.sendmessage = '인증번호가 발송 되었습니다.<br>이메일을 확인해주세요.';
		...
		proceedToNextPage() {
			const isValid = this.user.name && this.user.email && this.authCode && this.user.password && this.user.password === this.password_Check;
			if (isValid) {
			...
			<div class="login-container">
				...
				<div class="row mb-3">
					...
					<div class="col-sm-8">
						<div class="input-group">
							...
							<button class="w-btn w-btn-gra-anim w-btn-gra3" type="button" @click="IdCheck">아이디 중복확인</button>
						</div>
					</div>
				</div>
				<!-- 이름 입력 폼 -->
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="email" class="form-control" id="email" placeholder="이메일을 입력하세요"
								v-model="user.email" :style="{ 'background-color': inputemail }" @input="emailChange">
				...
				<div class="button-container">
					<button type="button" class="btn btn-primary" @click="proceedToNextPage"
						:disabled="!user.id || !user.name || !user.email || !authCode || !user.password || !password_Check && (user.password !== password_Check)">다음</button>
				</div>
			</div>
		</div>
	</main>
